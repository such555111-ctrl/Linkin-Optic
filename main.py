...
log = logging.getLogger("linkin_optic")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "LINKIN OPTIC: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "🕶️ *LINKIN OPTIC — Eyewear Boutique*\n\n"
        "Добро пожаловать в LINKIN OPTIC! Оригинальные солнцезащитные и "
        "оптические очки мировых брендов с доставкой по всей России.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ LO-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от LINKIN OPTIC по заказу LO-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от LINKIN OPTIC по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
